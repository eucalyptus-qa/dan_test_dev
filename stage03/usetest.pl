...
if (@ids < 1 || !$done) {
    print "ERROR: could not get ids from ec2-describe-instances\n";
    exit(1);
}
...
if ($managed) {
	...
	$cmd = "runat $ec2timeout ec2-describe-addresses | grep admin";
    	...
    	close(RFH);
    	if (@ips < 1) {
		print "ERROR: could not get any addrs from ec2-describe-addresses\n";
		exit(1);
    	};
	...
	while( $kyo_count < $kyo_try ){    
    		system("date");
		$cmd = "runat $ec2timeout ec2-describe-instances $theid | grep INST | awk '{print \$4}'";
    		print "CMD: $cmd\n";
    		open(RFH, "$cmd|");
    		while(<RFH>) {
			chomp;
...
#    system("date");
	...
	$cmd = "runat $ec2timeout ec2-disassociate-address $existingip";
...
#    }
	...
	$cmd = "runat $ec2timeout ec2-associate-address $theip -i $theid";
...
#    print "INSTANCE: $ids[$i], $privips[$i], $keys[$i], $states[$i]\n";
	...
	if (!$done) {
	    system("date");
	    $cmd = "runat $ec2timeout ec2-get-console-output $theid";
	    $rc = system("$cmd > out.$$");
	    if (!$rc) {
		$cmd = "grep -i sshd out.$$";
		$rc = system("$cmd");
